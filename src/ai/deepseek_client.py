...
class DeepSeekClient:
    ...
    def chat_completion(
        ...
        try:
            ...
            logger.debug(f"DeepSeek API响应: {json.dumps(result, ensure_ascii=False)[:200]}")
            ...
            logger.warning(f"⏰ DeepSeek API请求超时（{actual_timeout}秒）")
    ...
    def chat(
        ...
        if result['success']:
            try:
                ...
                logger.debug(f"DeepSeek AI响应: {content[:200]}...")
                return content
            except (KeyError, IndexError) as e:
                logger.error(f"解析DeepSeek响应失败: {e}, 原始数据: {result['data']}")
                return None
        else:
            logger.error(f"DeepSeek API调用失败: {result.get('error_message', result.get('error'))}")
            return None
    ...
    def chat_json(
        ...
        if result['success']:
            try:
                content = result['data']['choices'][0]['message']['content']
                logger.debug(f"DeepSeek AI JSON响应: {content[:200]}...")
                # 解析JSON
            ...
            logger.error(f"DeepSeek API调用失败: {result.get('error_message', error_type)}")
    ...
    def get_balance(self) -> Dict:
        ...
        try:
            ...
            result = response.json()
            logger.debug(f"DeepSeek余额查询响应: {json.dumps(result, ensure_ascii=False)}")
            ...
            logger.warning(f"⏰ DeepSeek余额查询请求超时")
            return {
                'success': False,
                'error': 'timeout',
                'error_message': '请求超时'
            ...
            logger.error(f"DeepSeek余额查询请求失败: {e}")
            return {
                'success': False,
                'error': 'request_failed',
                'error_message': str(e)
            ...
            logger.error(f"DeepSeek余额查询异常: {e}")
            return {
                'success': False,
                'error': 'unknown_error',
                'error_message': str(e)
...
if __name__ == '__main__':
    ...
    # 从.env读取DeepSeek配置（需要添加到settings.py）
    ...
    if deepseek_api_key:
        ...
        print("请配置DeepSeek API密钥（环境变量: DEEPSEEK_API_KEY）")
